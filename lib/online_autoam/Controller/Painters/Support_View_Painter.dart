...
class Support_View_Painter extends CustomPainter{

  late Support_Filler  support_filler;
  late Piece_model piece_model;


  Support_View_Painter(this.support_filler, this.piece_model);
...
double scale=200/piece_model.piece_height;
  ...
  draw_front(Canvas canvas, double scale) {
    ...
    Path p=Path();
     ...
     piece_w = piece_model .piece_width * scale;
     piece_h = piece_model .piece_height * scale;
    ...
    late Offset filler_origin=front_origin;

    ///vertical
    if(support_filler.vertical){
      if(support_filler.face){
        filler_w=support_filler.width*scale;
        filler_h=piece_h;
      }else{
        filler_w=support_filler.thickness*scale;
        filler_h=piece_h;
      }
    }
    /// horizontal
      ...
      if(support_filler.face){
        filler_w=piece_w;
        filler_h=support_filler.width*scale;
      }else{
        filler_w=piece_w;
        filler_h=support_filler.thickness*scale;
      }
    }
